/**
 * BSC Candidate CRM — Shared JS
 * Auth · API · Nav · Utils · Toast · Modal
 */
'use strict';
...
const Auth = {
  save(role, username) {
    localStorage.setItem(CONFIG.SESSION_KEY, JSON.stringify({ role, username, loginAt: Date.now() }));
  },
  get() {
    try { return JSON.parse(localStorage.getItem(CONFIG.SESSION_KEY)); } catch { return null; }
  },
  check() { return !!this.get(); },
  logout() { localStorage.removeItem(CONFIG.SESSION_KEY); window.location.replace('login.html'); },
  guard() {
    if (!this.check()) {
      if (!sessionStorage.getItem('_bsc_redirecting')) {
        sessionStorage.setItem('_bsc_redirecting','1');
        setTimeout(() => sessionStorage.removeItem('_bsc_redirecting'), 3000);
        window.location.replace('login.html');
      }
      return null;
    }
    sessionStorage.removeItem('_bsc_redirecting');
    return this.get();
  },
...
const API = {
  async call(action, params = {}) {
    ...
    try {
      ...
      console.warn('[BSC API] Fallback to sample —', err.message);
      return this._sample(action, params);
    }
  },
  ...
  _sample(action, params) {
    ...
    return Promise.resolve(s[action] || { success:true });
  }
};
